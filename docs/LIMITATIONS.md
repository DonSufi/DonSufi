...
- **Gap**: there is no background task (e.g. `expo-background-fetch` / `expo-task-manager`) wired up to refresh that window automatically if the app isn't opened for several days. A user who doesn't open the app for 4+ days could see notifications lapse until they next open it.
- **To close**: register a periodic background task that calls `rescheduleAdhanNotifications` (already idempotent and cheap — it diffs against what's scheduled) on a daily cadence, and test its actual reliability on both platforms, since both iOS and Android impose OS-level restrictions on background execution that vary by device and battery-optimization settings.
