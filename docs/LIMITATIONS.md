...
- **A real gap was found and partially fixed here**: earlier translation content existed in the locale JSON files, but most screens never actually called `useTranslation()` -- they rendered hardcoded English strings regardless of the selected language, so even Arabic's complete translation catalog sat mostly unused. This has been fixed for the highest-impact path: onboarding (all 7 steps), the shared location picker (used in onboarding and Settings), the tab bar, all native screen-header titles, and the language-picker screen itself. A regression test (`src/i18n/__tests__/locales.test.ts`) now checks every locale file for key parity, orphaned keys, and empty strings, so future translation work can't silently drift.
- **Still hardcoded in English regardless of language setting**: the Ramadan, Du'a, Islamic Calendar, Prayer Tracker, Mosques, and Qur'an screens, plus five Settings sub-screens (Prayer, Notifications, Appearance, Accessibility, Privacy). These need the same `useTranslation()` wiring the fixed screens now have, plus the corresponding namespace(s) added to all 11 locale files.
- **English and Arabic have complete translation *content*** for everything currently wired up, and Arabic's content is complete for the full key set (verified by the parity test). Nine more languages (French, Urdu, Turkish, Indonesian, Malay, Spanish, Italian, German, Bengali) cover onboarding, navigation, and core screens; everything else falls back to English via i18next. None of the translations (including Arabic) have had a professional linguist review -- a native speaker should check each before an app store listing calls it "complete."
